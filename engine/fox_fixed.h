...
extern qFixed16_t cosTable[ANGLE_COUNT];
extern qFixed16_t focalTable[ANGLE_COUNT];
...
static inline qFixed24x8_t fcos24(int angle) { return (qFixed24x8_t)(cosTable[angle & (ANGLE_COUNT - 1)] >> 8); }

static inline qFixed24x8_t fixed_int24(int x) { return x << FIXED8_SHIFT; }
static inline qFixed16_t fixed_int16(int x) { return x << FIXED16_SHIFT; }
...
static inline void initFocalTable(void) {
    for (int i = 1; i < ANGLE_COUNT; i++) {
        float angle = (2.0f * M_PI * i) / ANGLE_COUNT;
        focalTable[i] = to_fixed16(1.0f / tanf(angle * 0.5f));
    }
